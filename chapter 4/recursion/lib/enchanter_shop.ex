defmodule EnchanterShop do
  def test_data do [
    ...
    # con filtro, clausula intermedia entre la base y la transformacion que
    # filtra mediante map pattern matchin en magic
    def enchant_for_sale([]), do: []
    ...
    def enchant_for_sale([item | incoming_items]) do
    new_item = %{
    title: "#{@enchanter_name}'s #{item.title}", price: item.price * 3,
    magic: true
      }
      [new_item | enchant_for_sale(incoming_items)]
    end
